...
public:
	...
	void enemy_gets_damaged(Enemy_Manager EN, weaponManager W, Tools_manager &tm, std::vector <Ammo*> &a, std::vector <Tools*> &t,
		std::vector <enemies*> &f, std::vector <Weapon*> &l, Status &s, ALLEGRO_SAMPLE_INSTANCE *instance, ALLEGRO_SAMPLE_INSTANCE *hit_sound, ALLEGRO_FONT *font, Animate &hit, int &score);
	...
	void enemy_dies( ALLEGRO_FONT *font, Animate &d, Status &s, int &numkills, int &score, Tools_manager & tm, std::vector<Ammo*>& a, std::vector<Tools*>& t,
		std::vector<enemies*>& f, ALLEGRO_SAMPLE_INSTANCE * instance);
	...
	void Ball_gets_destroyed(Enemy_Manager & EN, weaponManager W, Tools_manager & tm, std::vector<Molten_shot*>& b, std::vector<Weapon*>& f, std::vector <Ammo*> &a, 
		ALLEGRO_SAMPLE_INSTANCE * instance, bool & bossdefeated);

	void Boss_weapon_immune_to_weapon(Enemy_Manager & EN, Stages s, std::vector <enemies*> e, weaponManager W, std::vector <Diamond_shot*> &ds,
		...
		std::vector <Molten_shot*> &mball, std::vector <v_beam*> &v, std::vector <Enemy_Lazer*> &lazer, std::vector<Weapon*>& f,
		std::vector <Turrets *> turrets, ALLEGRO_SAMPLE_INSTANCE * instance, bool & bossdefeated);

	void Boss_weapon_immune_to_weapon(Enemy_Manager & EN, weaponManager W, std::vector <enemies*> e, std::vector<Weapon*>& f, ALLEGRO_SAMPLE_INSTANCE * instance, bool & bossdefeated);
	...
	void player_gets_damaged(Enemy_Manager &EN, Stages stage, std::vector <enemies*> &foes, std::vector <Spartak_ball*> &b, std::vector <Diamond_shot*> &ds,
		std::vector <Egg_Bomb*> &EB, std::vector <Sonic_Turbulence*> &ST, std::vector <Lazer_B*> &LB, std::vector <Kamet*> &K,
		std::vector <Heat_Wave*> &hw, std::vector <Single_Twin*> &st, std::vector <Spartak_Laser*> &sl,
